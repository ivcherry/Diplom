                ...
                <div class="modal-body">
                    ...
                    <div class="row form-inline">
                            ...
                            <div class="roles-form">
                                @foreach ( $roles as $role )
                                    @if ( $userModel->getRoles()->contains($role) )
                                        <div><input type="checkbox" name=" {{ $role->getName() }} "
                                                    id=" {{ $role->getId() }} " value=" {{ $role->getId() }} "
                                                    checked> {{ $role->getDescription() }} </input></div>
                                    @else
                                        <div><input type="checkbox" name=" {{ $role->getName() }} "
                                                    id=" {{ $role->getId() }} "
                                                    value=" {{ $role->getId() }} "> {{ $role->getDescription() }} </input>
                                        </div>
                                    @endif
                                @endforeach
                                <div>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-modal--success" id="saveEditRoles">Сохранить</button>
                            <button type="reset" id="cancelEdit" class="btn btn-modal--danger" data-dismiss="modal">
                                Отмена
                            </button>
                        </div>
                    </div>
